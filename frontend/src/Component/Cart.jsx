...
const Cart = ({ name, seller, color, price, image, removeIcon, handleRemove, onQuantityChange }) => {
  const [quantity, setQuantity] = useState(1);

  const handleQuantityChange = (type) => {
    setQuantity((prev) => {
      const newQuantity = type === "increase" ? prev + 1 : Math.max(1, prev - 1);
      const quantityDifference = newQuantity - prev;
      onQuantityChange(quantityDifference, price);
      return newQuantity;
    });
  };
  ...
  return (
    <div className="bg-white flex w-[821px] h-[220px] rounded-lg shadow-md p-2 relative">
      ...
      <div className="absolute top-[25%] right-[50px] transform -translate-y-1/2">
        <span className="text-lg font-bold text-black">${(price * quantity).toFixed(2)}</span>
      </div>
...
// // eslint-disable-next-line react/prop-types
// const Cart = ({ id, name, seller, color, price, image, removeIcon, quantity, handleRemove, onQuantityChange }) => {
//   const handleQuantityChange = (type) => {
//     if (type === "increase") {
//       onQuantityChange(1);
